...
function WorldExitDialog.ShowPage(callback)
    ...
    local function Handle()
        ...
        local width = 660
        local height = 420
        local url = "Mod/WorldShare/cellar/WorldExitDialog/WorldExitDialog.html"

        if System.options.isEducatePlatform then
            url = "script/apps/Aries/Creator/Game/Educate/Other/WorldExitDialog.html"
        end
    ...
    if GameLogic.IsReadOnly() then
        ...
        if KeepworkServiceSession:IsSignedIn() then
            Compare:Init(currentEnterWorld.worldpath, function(result)
                if not result then
                    GameLogic.SendErrorLog("WorldExitDialog","exit world error","world path====="..(currentEnterWorld.worldpath or ""))
                    return
                end
...
function WorldExitDialog.SetWindowText()
