...
function SyncWorld:SyncToDataSource(callback, notShowFinishPage)
    local currentWorld = Mod.WorldShare.Store:Get('world/currentWorld')
	if(not currentWorld) then
		-- Alert: sync with empty world?
		if(callback) then
			callback()
		end
		return
        ...
        local syncInstance = SyncToDataSource:Init(function(result, option)
            if result == false then
                ...
                if option and type(option) == 'table' then
                    ...
                    if option.method == 'UPDATE-PROGRESS-FINISH' then
                        if notShowFinishPage then
                            Progress:UpdateDataBar(1, 1, L'处理完成')
                            Progress:ClosePage()
                            GameLogic.GetFilters():apply_filters('SyncWorldFinishNoUI')
                        else
                            Progress:SetFinish(true)
                            Progress:UpdateDataBar(1, 1, L'处理完成')
                        end
    ...
    KeepworkServiceWorld:GetWorldsList(function(data)
        ...
        if dataCount >= 3 and not (System.options.channelId and System.options.channelId ~= "") then
...
function SyncWorld:CheckAndUpdatedByFoldername(folderName, callback,from)
    self.from = from
    KeepworkServiceProject:GetProjectIdByWorldName(folderName, false, function(projectId)
        ...
        if not ParaIO.DoesFileExist(worldPath) or not ParaIO.DoesFileExist(worldPath..'tag.xml') then
            worldPath =
                format(
                    '%s/%s/',
...
function SyncWorld:CheckAndUpdated(callback)
    ...
    if currentWorld.status == 5 then
        if self.from and self.from == "papa_adventure" then
            SyncWorld:SyncToLocal(callback, nil, true)
            self.from = nil
            return
        end
