...
function Progress:ShowFinishPage()
    ...
    if System.options.isEducatePlatform then
        ...
        if self.syncInstance and self.syncInstance.Close then
            self.syncInstance:Close({ fromButton = 'close' })
        end
        return
    end
    ...
    KeepworkServiceProject:GenerateMiniProgramCode(
        ...
        function(bSucceed, wxacode)
            ...
            if System.options.isPapaAdventure then
                ...
                if Creation.needExitWorld then
                    Creation.needExitWorld = false
                    ...
                    GameLogic.GetFilters():apply_filters('SyncWorldFinishNoUI',currentWorld.kpProjectId)
                end
            end
        end
    )
...
function Progress:CloseFinishPage(fromButton)
    ...
    if FinishPage then
        ...
        GameLogic.GetFilters():apply_filters('SyncWorldEnded')
    end
end
