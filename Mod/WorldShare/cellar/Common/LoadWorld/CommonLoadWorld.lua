...
function CommonLoadWorld:CheckLoadWorldFromCmdLine(cmdLineWorld)
    ...
    if paramWorld and paramWorld == cmdLineWorld then
        ...
        if loginEnable then
            if MainLogin.canEnterWithoutSignIn then
                MainLogin.canEnterWithoutSignIn = nil
                enterFunc()
            else
                if KeepworkServiceSession:IsSignedIn() then
                    enterFunc()
                else
                    MainLogin.canEnterWithoutSignIn = true
                    MainLogin:Show()
    ...
    if not System.os.IsEmscripten() and cmdLineWorld:match('^https?://') then
        GameLogic.RunCommand(format('/loadworld -auto %s', cmdLineWorld))
        return
    end

    local function Handle(bAutoLogin)
        if (System.os.IsEmscripten()) then
            NPL.load("(gl)script/apps/Aries/Creator/Game/Emscripten/Emscripten.lua");
        end

        if System.os.IsEmscripten() and cmdLineWorld:match('^https?://') then
            GameLogic.RunCommand(format('/loadworld -auto %s', cmdLineWorld))
            return
        end
        -- LOG.std(nil, 'info', 'CommandLoadWorld', 'cmdLineWorld==========: %s,%s', cmdLineWorld,tostring(System.options.isEducatePlatform))
        ...
        if System.options.isEducatePlatform and isEducateCommand then
            ...
            -- LOG.std(nil, 'info', 'CommandLoadWorld', "myKey:========="..myKey.."，cmdline_secretkey========="..(System.options.cmdline_secretkey or ""))
            -- LOG.std(nil, 'info', 'CommandLoadWorld', "token=========="..(System.options.cmdline_token or ""))
            -- LOG.std(nil, 'info', 'CommandLoadWorld', "userId=========="..userId)
        ...
        -- LOG.std(nil, 'info', 'CommandLoadWorld', 'projectId==========: %d', (id or 0))
        if not id then
            if cmdLineWorld:match("loadworld") then
                GameLogic.RunCommand(cmdLineWorld)
                return
            end
            if cmdLineWorld:find("edu_do_works") then
                if WisidomEducate then
                    WisidomEducate.RunEduCommands(cmdLineWorld)
                end
                return
            end
            ...
            if not Mod.WorldShare.Store:Get('world/isEnterWorld') then
                ...
                if System.options.isEducatePlatform and bAutoLogin then
                    commonlib.TimerManager.SetTimeout(function()
                        MainLogin:Next()
                    end,100)
                end
        ...
        if System.options.isPapaAdventure then
            ...
            if GameMainLogin and type(GameMainLogin.SetFutureLoadworld) == "function" then
                GameMainLogin:SetFutureLoadworld(id,true)
                return
            end
        end
        local commandStr = '/loadworld -auto %d'
        KeepworkServiceProject:GetProject(
            id,
            function(data, err)
                if data and type(data) == 'table' and data.isFreeWorld == 1 then
                    GameLogic.RunCommand(format(commandStr, id))
                else
                    if type(data) == 'table' then
                        if System.options.isEducatePlatform then
                            if Game.is_started then
                                commandStr = '/loadworld -auto -s %d'
                            end

                            if isEducateCommand and data.visibility == 1 then
                                commandStr = '/loadworld -auto -lesson %d'
                                if Game.is_started then
                                    commandStr = '/loadworld -auto -s -lesson %d'
                                end
                            end
                            LOG.std(nil, 'info', 'CommandLoadWorld', 'commandStr==========: %s', commandStr)
                        end
                    end
                    if not Mod.WorldShare.Store:Get('world/isEnterWorld') then
                        if KeepworkServiceSession:IsSignedIn() then
                            ...
                            --     Game.MainLogin.cmdWorldLoaded = false
                            -- else
                            --     GameLogic.RunCommand(format(commandStr, id)) -- show error msg
                            --     System.options.cmdline_world = nil
                            --     Game.MainLogin:next_step({IsLoginModeSelected = false})
                            -- end
                        end
    ...
    local urlProtocol = UrlProtocolHandler:GetParacraftProtocol(cmdLineWorld) or ''
    ...
    local userToken = urlProtocol:match('usertoken="([%S]+)"') or System.options.cmdline_token
    if userToken then
        ...
        if token == userToken then
            ...
            MainLogin:LoginWithToken(userToken, function(bIsSuccessed, reason, message)
                if not bIsSuccessed then
                    if message:find('(401)') and System.options.isEducatePlatform  then
                        GameLogic.AddBBS(nil,L"登录状态失效，请重新登录")
                        LoginModal:CheckSignedIn(L'该项目需要登录后访问', function(bIsSuccessed)
                            if bIsSuccessed then
                                Handle(true)
                            else
...
function CommonLoadWorld.GotoUrl(url)
    ...
    local projectId, classId = url:match("^(%d+)(%a+%d+)")

    if projectId and classId then
        System.User.worldclassid = string.lower(classId)
        GameLogic.RunCommand('/loadworld -auto ' .. projectId)
    ...
    if urlObj and urlObj:IsRemoteServer() then  
        if ServerSetting and ServerSetting.IsConnectSelf(url) then
            GameLogic.AddBBS(nil,L"连接服务器失败，不可连接自己")
            return
        end    
        ServerSetting.CheckUserAuth(function()
...
function CommonLoadWorld:EnterCacheWorldById(pid)
    ...
    local fileUrl = world:GetLocalFileName()
    if fileUrl then
        if(ParaIO.DoesFileExist(fileUrl, true)) then
        ...
        commonlib.SendErrorLog("CommonLoadWorld","load world failed","load world failed============")
        return false;
    end
end
...
function CommonLoadWorld:EnterWorldById(pid, refreshMode, failed,isLesson, isSilent)
    print("EnterWorldById",pid, refreshMode, failed,isLesson, isSilent)
    if not pid then
        return
    end
    ...
    local function HandleLoadWorld(worldInfo, offlineMode)
        ...
        if cacheWorldInfo then
            -- see https://keepwork.com/official/docs/api/gitstore for details
            -- only latest commit is uploaded to CDN source
            local qiniuZipArchiveUrl = GitKeepworkService:GetQiNiuArchiveUrl(
                                        worldInfo.worldName,
                                        worldInfo.username,
                                        cacheWorldInfo.worldInfo.commitId)
            -- this is fetched with source in our own server.
            local cdnArchiveUrl = GitKeepworkService:GetCdnArchiveUrl(
                                    worldInfo.worldName,
                                    worldInfo.username,
                                    cacheWorldInfo.worldInfo.commitId,
                                    (isLesson == true and worldInfo.projectId or nil))
    ...
    -- offline mode
    local cacheWorldInfo = CacheProjectId:GetProjectIdInfo(pid)
    -- not System.os.IsEmscripten() 非Emscirpten走离线逻辑, Emscripten 不走离线逻辑
    if (System.options.loginmode == 'local') and
    not KeepworkServiceSession:IsSignedIn() and not System.os.IsEmscripten() and
    cacheWorldInfo then
            ...
            local function HandleVerified()
                ...
                if data and
                   ...
                   not self.systemGroupMemberVerified then
                    if (not data.level or data.level == 0 )and  false then
                        local username = Mod.WorldShare.Store:Get('user/username')
                        GameLogic.GetFilters():apply_filters("enter_world_fail",pid)
                        GameLogic.AddBBS(
                            nil,
                ...
                if data and
                   ...
                   not self.encodeWorldVerified then
                    if not KeepworkServiceSession:IsSignedIn() then
                        ...
                        if (data.username and data.username == username) or System.options.isEducatePlatform then
