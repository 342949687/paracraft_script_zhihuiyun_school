...
-- update System.User.isAnonymousWorld according to current world info. 
function KeepworkServiceWorld:RefreshAnonymousWorld(world)
    local paramWorld = ParaEngine.GetAppCommandLineByParam('world', nil)

	System.User.isAnonymousWorld = false;
	if(not world) then
		world = Mod.WorldShare.Store:Get('world/currentEnterWorld')
		if(not world) then
			return;
		end
	end
    if KeepworkServiceSession:IsSignedIn() then
        ...
        if currentWorld and currentWorld.worldpath == paramWorld then
            return
        end

        local username = Mod.WorldShare.Store:Get('user/username')
        if world.username ~= username then
            -- not world owner.
            if (not VipTypeWorld:IsFreeWorld(world)) then
                -- not white-listed free world.
                GameLogic.IsVip('Vip', false, function(result)
                    if not result then
						if(VipTypeWorld:IsVipWorld(world) or VipTypeWorld:IsInstituteVipWorld(world)) then
							-- for special worlds
						else
							-- TODO: figure out a way to filter out school or org user. Currently, it requires a web call to check permission which is slow and wrong. 
							-- System.User.isAnonymousWorld = true
						end
                    end
                end, 'Vip')
            end
		...
		-- for non-signed in user (offline mode)
		local worldpath = world.worldpath
        worldpath = worldpath and worldpath:gsub('[/\\]$', '')
        paramWorld = paramWorld and paramWorld:gsub('[/\\]$', '')
		
        if worldpath and worldpath ~= paramWorld then
			if(VipTypeWorld:HasProjectId(world) or VipTypeWorld:IsVipWorld(world) or VipTypeWorld:IsInstituteVipWorld(world)) then
				System.User.isAnonymousWorld = true    
			end
        end
    end
...
function KeepworkServiceWorld:GetWorld(foldername, shared, worldUserId, callback, isRefresh)
...
function KeepworkServiceWorld:MergeRemoteWorldList(localWorlds, callback,bNoFilter)
    ...
    local mergeFunc = function(remoteWorldsList,err,callback)
        ...
        for DKey, DItem in ipairs(remoteWorldsList) do
            ...
            -- shared world text
            -- xizhi and liyu requested to cancel this function. 2023.11.20
            -- if remoteShared and remoteWorldUserId ~= tonumber(userId) then
            --     if DItem.extra and DItem.extra.worldTagName then
...
function KeepworkServiceWorld:LimitFreeUser(isShowUI, callback, allowMax)
    ...
    -- BugID 1003007
    if (true) then return callback(true) end
