...
KeepworkServicePermission.AllAuth = {
    ...
    MultiPlayerNetwork = {
        key = 'multiplayer_network_services',
        desc = L'多人联网服务',
    }
}
...
function KeepworkServicePermission:Authentication(authName, callback)
    -- BugID 1003006 LimitWorldSize20Mb OnlineWorldData50Mb
    -- BugID 1003015 PythonCodeBlock
    ...
    KeepworkPermissionsApi:Check(
        ...
        function(data, err)
            if (data and type(data) == "number" and data == 0) or (data and type(data) == "table" and data.data == true) then
                if callback and type(callback) == 'function' then
