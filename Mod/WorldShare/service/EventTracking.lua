...
function EventTrackingService:Init()
    ...
    self.firstInit = true
    self.timeInterval = 60000 -- 60 seconds
...
function EventTrackingService:GenerateDataPacket(eventType, userId, action, started,use_new_catalog)
    ...
    if eventType == 1 then -- one click event
        return {
            ...
            use_new_catalog = use_new_catalog
        }
...
-- eventType: 1 is one click event, 2 is duration event,use_new_catalog:true or false
function EventTrackingService:Send(eventType, action, extra, offlineMode,use_new_catalog)
    ...
    local dataPacket = self:GenerateDataPacket(eventType, userId, action, extra and extra.started or false,use_new_catalog)
    ...
    local catalog = use_new_catalog and "new-behavior" or "behavior"
                        ...
                        if unitinfo and type(unitinfo) == 'table' then
                            for uKey, uItem in ipairs(unitinfo) do
                                if uItem and uItem.packet then
                                    ...
                                    EventGatewayEventsApi:Send(
                                        uItem.use_new_catalog and "new-behavior" or "behavior",
                                        uItem.action,
                                        uItem.packet,
                                        nil,
                                        function(data, err)
