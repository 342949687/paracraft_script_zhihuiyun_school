...
function SocketService:StartUDPService()
	...
	LOG.std(nil, "info", "SocketService", "start UDP service")
...
function SocketService:SendUDPIamOnlineMsg(ip, port)
	...
	local serverInfo = ServerSetting.GetServerInfo() or {}
	self:SendUDPMsg(
		{
			type = "ONLINE",
			result = "IAM",
			username = (serverInfo.creator and serverInfo.creator ~= "") and serverInfo.creator or "nil",
			serverName = (serverInfo.name and serverInfo.name ~= "") and serverInfo.name or "nil",
			playerCount= serverInfo.playerCount or 1,
			password= serverInfo.password or "",
			uuid = self.uuid,
...
function SocketService:ReceiveUDPMsg(msg)
	...
	if msg.type == 'ONLINE' then
		...
		if msg.result == "IAM" then
			...
			udpServerList[#udpServerList + 1] = { ip = udpIp, port = msg.worldPort, username = msg.username, serverName = msg.serverName, uuid = msg.uuid ,password = msg.password, playerCount = msg.playerCount}
