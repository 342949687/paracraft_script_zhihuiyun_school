...
-- return: object
function KeepworkProjectsApi:CreateProject(foldername, platform, success, error)
    local url = '/projects'
...
-- method: DELETE
-- return: object
function KeepworkProjectsApi:RemoveProjectWithNoPWD(kpProjectId, success, error)
    if not kpProjectId or
       type(kpProjectId) ~= 'number'  then
        return
    end

    local url = format('/projects/%d', kpProjectId)
    KeepworkBaseApi:Delete(url, nil ,success, error)
end
