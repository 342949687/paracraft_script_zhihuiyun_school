...
function WorldShare:init()
    ...
    if((System.options.cmdline_world or "") == "" and not System.options.servermode) then
    ...
    -- get time from server when app started.
    KeepworkServiceSession:GetCurrentServerTime()

    -- big 2023.3.4: Temporary logic, will be removed in subsequent versions.
    if (System.os.GetPlatform() == 'mac' or
        System.os.GetPlatform() == 'ios') then
        local function CompareParaEngineVersion(ver)
        ...
        if (not CompareParaEngineVersion('1.3.1.0')) then
            local jumpUrl = ''

            if (System.os.GetPlatform() == 'mac') then
                jumpUrl = 'itms-apps://itunes.apple.com/app/id1422406070'
            elseif (System.os.GetPlatform() == 'ios') then
                jumpUrl = 'itms-apps://itunes.apple.com/app/id1422411677'
            end
