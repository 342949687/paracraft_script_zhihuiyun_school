...
function Filters:Init()
    ...
    GameLogic.GetFilters():add_filter('ShowClientUpdaterNotice', function()
    ...
    GameLogic.GetFilters():add_filter('SaveWorldPage.ShowSharePage', function(callback)
            ...
            if currentEnterWorld and
               string.match(currentEnterWorld.foldername, '.+_main') and
               generatorName == "paraworldMini" then
    ...
    GameLogic.GetFilters():add_filter('OnClickCreateWorld', function()
            return CreateWorld.OnClickCreateWorld()
        end
    )

    -- filter KeepworkPremission
    GameLogic.GetFilters():add_filter('KeepworkPermission',
    ...
    GameLogic:GetFilters():add_filter('LoginModal.CheckSignedIn', function(bEnabled, desc, callback)
    ...
    -- filter menu command
    GameLogic:GetFilters():add_filter('menu_command',
    ...
    GameLogic:GetFilters():add_filter('user_behavior', function(type, action, extra,offlineMode,use_new_catalog)
            EventTrackingService:Send(type, action, extra,offlineMode,use_new_catalog)
        end
    )
    ...
    -- filter is signed in
    GameLogic.GetFilters():add_filter('is_signed_in', function()
            return KeepworkServiceSession:IsSignedIn()
        end
    )

    -- filter set mode
    GameLogic.GetFilters():add_filter('set_mode', function(mode, bFireModeChangeEvent)
    ...
    GameLogic.GetFilters():add_filter('get_user_type', function()
            return Mod.WorldShare.Store:Get("user/userType")
        end
    )

    -- filter get user id
    GameLogic.GetFilters():add_filter('get_user_id', function()
            return Mod.WorldShare.Store:Get("user/userId") or 0
        end
    )

    -- filter get world by project id
    GameLogic.GetFilters():add_filter('get_world_by_project_id', function(projectId, callback)
            KeepworkServiceWorld:GetWorldByProjectId(projectId, callback)
        end
    )

    -- filter get keepwork url
    GameLogic.GetFilters():add_filter('get_keepwork_url', function()
            return KeepworkService:GetKeepworkUrl()
        end
    )

    -- filter get project id by lesson id
    GameLogic.GetFilters():add_filter('get_project_id_by_lesson_id', function(txtLessonId)
            return Mod.WorldShare.Utils.GetProjectId(txtLessonId)
        end
    )

    -- filter on exit
    GameLogic.GetFilters():add_filter('on_exit', function(bForceExit, bRestart, callback)
    ...
    GameLogic.GetFilters():add_filter('open_keepwork_url', function(url)
            Mod.WorldShare.Utils.OpenKeepworkUrlByToken(url)
        end
    )

    -- filter check signed in
    GameLogic.GetFilters():add_filter('check_signed_in', function(text, callback)
            LoginModal:CheckSignedIn(text, callback)
        end
    )

    -- filter show login page
    GameLogic.GetFilters():add_filter('show_login_page', function()
            LoginModal:Init()
        end
    )

    -- filter qiniu upload file
    GameLogic.GetFilters():add_filter('qiniu_upload_file', function(token, key, filename, content, callback)
            QiniuRootApi:Upload(token, key, filename, content, callback, callback)
        end
    )

    -- filter show create page
    GameLogic.GetFilters():add_filter('show_create_page', function()
            Create:Show()
    ...
    GameLogic.GetFilters():add_filter('check_unavailable_before_open_vip',function(param)
        param = param or {}
        local is_unavailable = System.options.isHideVip --是否禁用vip
