...
function GeneralGameClient:LoadWorld(opts)
    ...
    local isReloadWorld = opts.reload or (curWorldId and tostring(options.worldId) ~= tostring(curWorldId)); 
...
function GeneralGameClient:GetServerListByWorldId(worldId, callback)
    local netHandler = self:GetWorldNetHandler();
    if (not netHandler) then return end
    worldId = worldId or GameLogic.options:GetProjectId();
    netHandler:AddToSendQueue(Packets.PacketGeneral:new():Init({action = "Debug", data = { cmd = "ServerList"}}));
    netHandler.debug_serverlist_callback = function(worlds)
        local worldlist = {};
        -- local options = self:GetOptions();
        -- local curworld = {
        --     worldName = options.worldName,
        --     worldId = tostring(options.worldId),
        --     worldKey = options.worldKey,
        --     ip = options.ip,
        --     port = options.port,
        ...
        -- }
        -- worlds = worlds or {};
        -- worlds[curworld.worldKey] = worlds[curworld.worldKey] or curworld; 
        for key, world in pairs(worlds) do
            if (tostring(world.worldId) == tostring(worldId)) then
...
function GeneralGameClient:SwitchServer(world, bRestartWorld)
    world = world or {};
    world.reload = bRestartWorld;
    GGS.INFO(world);
    self:OnWorldUnloaded();
