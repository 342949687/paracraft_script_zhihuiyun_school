...
function NetClientHandler:handlePlayerLogin(packetPlayerLogin)
    ...
    if (self:GetClient():GetMainPlayerEntitySkin()) then 
        if System.options.isEducatePlatform then
...
function NetClientHandler:handleGeneral_Debug(packetGeneral)
    ...
    if (cmd == "ServerList") then
        local worlds = {};
        for _, server in pairs(debug) do
            for key, world in pairs(server.worlds or {}) do
                worlds[key] = world;
                world.ip = server.outerIp;
                world.port = server.outerPort;
            end
        end
        if (type(self.debug_serverlist_callback) == "function") then
            self.debug_serverlist_callback(worlds);
            return;
        end
    end
    GGS.INFO(cmd, debug);
