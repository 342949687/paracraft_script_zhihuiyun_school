...
function PrepareApp.ShowPage()
    ...
    if System.options.ZhyChannel ~= "" then
        url = "script/apps/Aries/Creator/Game/ZhiHuiYun/PrepareApp/PrepareAppZHYPage.html"
        if string.find(System.options.ZhyChannel, "competition") then
            url = "script/apps/Aries/Creator/Game/ZhiHuiYun/PrepareApp/PrepareAppPage.html"
        end
...
function PrepareApp.CheckAsset()
    ...
    local assets = _assetsList
    if System.options.channelId_431 then
        assets = _assetsList_431
    end
    local file_list = {}
...
function PrepareApp.SetBarProgress(percent)
    ...
    if System.options.ZhyChannel ~= "" then
        if string.find(System.options.ZhyChannel, "competition") then
            progress_fg.width = math.floor(1024*(percent*0.01))
            progress_fg.background = string.format("Texture/ZhiHuiYun/competition/Loading/di2_1024x13_32bits.png#0 0 %s 13;",progress_fg.width)
        else
            progress_fg.width = math.floor(581*(percent*0.01))
            progress_fg.background = string.format("Texture/ZhiHuiYun/Loading/di2_581x8_32bits.png#0 0 %s 8;",progress_fg.width)
        end
    else
