...
KeepWorkMallPage.grid_data_sources = {
    { enabled = true, name = "1_1", invented = true },
    { enabled = true, name = "1_2", invented = true },
    { enabled = true, name = "1_3", invented = true },
    { enabled = true, name = "1_4", invented = true },
    { enabled = true, name = "1_5", invented = true },
    { enabled = true, name = "1_6", invented = true },
    { enabled = true, name = "1_7", invented = true },
    { enabled = true, name = "1_8", invented = true },
    { enabled = true, name = "1_9", invented = true },
    { enabled = true, name = "1_10", invented = true },
    { enabled = true, name = "1_11", invented = true },
    { enabled = true, name = "1_12", invented = true }
};
...
function KeepWorkMallPage.LoadMore(index)
    if (not index or type(tonumber(index)) ~= "number" or not self.grid_data_sources or System.options.isHideVip) then
        return;
    end

    index = tonumber(index);
    local dataLength = #self.grid_data_sources;

    if (index >= dataLength) then
        if (self.listFrom == "search") then
            if (self.lastLoadIndex ~= index) then
                self.lastLoadIndex = index;
                self.curPage = self.curPage + 1;
                self.SearchProduct(self.search_keyworlds);
            end
        elseif (self.listFrom == "goods") then
            if (self.lastLoadIndex ~= index) then
                self.lastLoadIndex = index;
                self.curPage = self.curPage + 1;
                self.GetGoodsData(cur_classifyId);
            end
        end
    end
end

function KeepWorkMallPage.OnMouseWheel()
    if (not page) then
        return;
    end
    ...
    if (itemGrideViewNode) then
        if (itemGrideViewNode:GetChild('pe:treeview') and
            ...
            if control then
                curLine = math.ceil(control.ClientY / 170);
            end
        end
    end
    ...
    for key, item in ipairs(self.grid_data_sources) do
...
function KeepWorkMallPage.SearchProduct(keywords)
    ...
    if httpwrapper_version then -- and (httpwrapper_version == "LOCAL" or httpwrapper_version == "STAGE" or httpwrapper_version == "ONLINE")
        keepwork.mall.searchGoods({
            ...
            if err ~= 200 or not data or not data.hits then
                return;
            end
            ...
            if (self.curPage == 1) then
                self.listHits = data.hits or {};
                self.lastLoadIndex = 0;
            else
                for key, value in ipairs(data.hits) do
                    self.listHits[#self.listHits + 1] = value;
                end
            end

            KeepWorkMallPage.grid_data_sources = self.listHits;
...
function KeepWorkMallPage.GetGoodsData(classifyId, keyword, only_refresh_grid)
    ...
    keepwork.mall.goods.get({
        ...
        if (self.curPage == 1) then
            self.listHits = data.rows or {};
            self.lastLoadIndex = 0;

            if KeepWorkMallPage.IsSelectRec() then
                --插入占位数据
                for i=1,5 do
                    table.insert(self.listHits, 1, { baner = 1, id = -1, placeholder = true });
                end
            end
        else
            for key, value in ipairs(data.rows) do
                self.listHits[#self.listHits + 1] = value;
            end
        end

        self.grid_data_sources = self.listHits;
        self.dataLoaded = true;
        self.HandleDataSources();
        self.FlushView(only_refresh_grid);
    end);
end
...
function KeepWorkMallPage.HandleDataSources()
    ...
    if System.options.isHideVip then
        KeepWorkMallPage.grid_data_sources = commonlib.filter(KeepWorkMallPage.grid_data_sources, function (item)
            return item.isPublic == 1 or item.placeholder;
        end)
    end
...
function KeepWorkMallPage.OnClickBuy(item_data)
    ...
    if (item_data.isModelProduct and item_data.bag_nums and item_data.bag_nums > 0) or item_data.vip_enabled == false then
        ...
        if model_url:match("^https?://") then
            ...
            if hideinhand then
                GameLogic.GetFilters():add_filter(eventname, KeepWorkMallPage.OnInstalledModel);
            end
...
function KeepWorkMallPage.OnInstalledModel(msg)
    GameLogic.GetFilters():remove_filter("installed_model",KeepWorkMallPage.OnInstalledModel);
