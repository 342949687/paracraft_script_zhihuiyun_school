...
function GameLogic.LoadGame()
	...
	-- 压测版本不卸载资源了 这个有可能是临时性代码 后面估计要去掉判断
...
function GameLogic.ToggleDesktop(name)
	if(name == "esc") then
		...
		if (System.options.isPapaAdventure and Creation.rejectEsc) or System.options.isZhihuiyunSchoolPackage then
			return
		else
...
function GameLogic.IsVip(name, bOpenUIIfNot, callbackFunc, uiType)
	...
	local bEnabled = 
		GameLogic.GetFilters():apply_filters(
			...
			bOpenUIIfNot,
			function(result)
				result = name == "PythonCodeBlock" and true or result;
...
function GameLogic.GetMachineID(oldId)
