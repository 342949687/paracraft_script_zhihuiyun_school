...
function Desktop.OnReceiveGSLChatMsg(nid, value)
	...
	if(sFirstLetter=="{")then
		value = ChatMessage.DecompressMsg(value);
		if(value and value.ChannelIndex == 1) then
			-- 2023.7.25: skip all non public channel messages to be shown in chat window
			local nid = value.from;
			if(not (nid and (MyCompany.Aries.Friends.IsFriendInMemory(nid) or nid == System.App.profiles.ProfileManager.GetNID()))) then
				value.skip_in_chatwnd = true
			end
		end
