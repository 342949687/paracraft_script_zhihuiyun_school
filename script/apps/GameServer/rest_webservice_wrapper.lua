...
function client.JsonTranslator(msg, url)
	...
	if(not msg.data or NPL.FromJson(msg.data, out)) then
		...
		if(strung and msg.data) then
			local data = strung.gsub(msg.data, "\0", "")
			if(data ~= msg.data and data) then
				LOG.std(nil, "warning", "REST", "remove \0 in json msg "..LOG.tostring("url: %s|", tostring(url))..LOG.tostring(msg.header)..LOG.tostring("|--> data|")..LOG.tostring(msg.data));
				if(NPL.FromJson(data, out)) then
					return out;
				end
			end
		end
