...
-- build main_full.pkg
function BuildParaWorld.BuildCompleteWithScriptPath(script_dir)
	...
	--make zip
	local _txtPathList = {}
	for k,v in pairs(pkg.txtPathList) do 
		table.insert(_txtPathList,script_dir..v)
	end
	PackageMakerPage.BuildPackageByGroupPath(pkg.txtPathList,pkg.src,script_dir)
	time_zip = _getUsedTime()
	
	--main_complete.zip to main_complete.pkg
	ParaAsset.GeneratePkgFile(script_dir..pkg.src, script_dir..pkg.dest);
	time_pkg = _getUsedTime()
	

	--main_complete.pkg to main_full.pkg
	local pkgPath = script_dir..pkg.dest
	if ParaIO.DoesFileExist(pkgPath) then
		local newPkgPath = string.gsub(pkgPath,"main_complete.pkg","main_full.pkg")
		ParaIO.MoveFile(pkgPath,newPkgPath)
	end
