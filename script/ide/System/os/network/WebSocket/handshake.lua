...
Modify Date: 2024.1.29
Desc: Following Websocket RFC: http://tools.ietf.org/html/rfc6455
-----------------------------------------------
NPL.load("(gl)script/ide/System/os/network/WebSocket/frame.lua");
-----------------------------------------------
]]
...
local http_headers = function(request)
  local headers = {};
  if (not request or not request:match('.*HTTP/1%.1')) then
    return headers;
  end
