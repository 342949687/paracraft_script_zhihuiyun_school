...
local Vol = gettable("ConvNet.Vol");
local ConvNet = gettable("ConvNet");
...
function ConvNet.augment(V, crop, dx, dy, flipLeftRight)
    ...
    -- dx = dx or (ConvNet.randi(0,(V.sx - crop)) * (math.random() < 0.5 and 1 or -1));
    -- dy = dy or (ConvNet.randi(0,(V.sy - crop)) * (math.random() < 0.5 and 1 or -1));
    local W;
